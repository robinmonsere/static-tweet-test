 ...
 */
export const getTweet = async (id, fetchOptions) => {
...
export const renderTweets = async (content, fetchedTweets) => {
    // find all data-tweet-id fields
    const tweetIds = content.match(/data-tweet="(\d+)"/g)?.map((s) => s.match(/\d+/)?.[0]);
